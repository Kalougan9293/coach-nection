...
import {
  ...
  prepareDepartementsForSupabase,
...
export default function DemandeForm() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (type === "checkbox") {
      ...
      setFormData(prev => {
        const next = { ...prev, [name]: type === 'range' ? parseInt(value) : value };
        if (name === 'ville') {
          const extracted = extractDepartementsFromVille(value);
          if (extracted.length > 0) {
            setDepartements((current) => mergeDepartements(current, extracted));
            setInvalidFields((prevInvalid) => {
              const n = new Set(prevInvalid);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (departements.length === 0) {
      setError("Veuillez indiquer au moins un département (détecté depuis la ville ou ajouté manuellement).");
      setInvalidFields(new Set(['departements', 'ville']));
      return;
    }

    const finalDepartements = prepareDepartementsForSupabase(departements);
    if (finalDepartements.length === 0) {
      setError("Les départements saisis ne sont pas valides. Utilisez des codes à 2 chiffres (ex. 75, 92, 2A).");
      setInvalidFields(new Set(['departements', 'ville']));
      return;
    }
        ...
        <div className="p-8">
          ...
          {step === 1 && (
            <div className="space-y-6 animate-fade-in">
              ...
              <div className={`rounded-xl p-4 transition-colors ${invalidFields.has('ville') || invalidFields.has('departements') ? 'border-2 border-red-500 bg-red-50' : ''}`}>
                ...
                <p className="text-xs text-gray-500 mt-2 mb-3">
                  Le département est détecté automatiquement (ex. &quot;Paris 15e&quot; → 75). Vérifiez ou complétez ci-dessous.
                </p>
                <label className="block text-xs font-bold text-[#1F2957] mb-2">Département(s) pour le matching *</label>
                <input
                  type="text"
                  value={deptInput}
                  onChange={handleDeptInputChange}
                  placeholder="Saisir 75, 92, 2A..."
                  className="w-full px-4 py-2 rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#D4DC53] outline-none transition-all text-sm mb-2"
                />
                {departements.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {departements.map((dep) => (
                      <span key={dep} className="flex items-center gap-2 px-3 py-1.5 bg-[#D4DC53] text-[#1F2957] text-sm font-bold rounded-full">
                        Dép. {dep}
                        <button type="button" onClick={() => handleRemoveDept(dep)} className="hover:text-red-600" aria-label={`Retirer ${dep}`}>×</button>
                      </span>
                    ))}
                  </div>
                ) : (
                  <p className="text-xs text-amber-700">Aucun département détecté — saisissez un code à 2 chiffres.</p>
                )}
              </div>
          ...
          <div className="mt-10 flex justify-between items-center pt-6 border-t border-gray-100">
            ...
            {step === 1 ? (
              <button type="button" onClick={() => {
                ...
                if (prepareDepartementsForSupabase(departements).length === 0) missing.push('departements');
